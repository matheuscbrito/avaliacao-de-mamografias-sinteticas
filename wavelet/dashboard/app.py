...
FEATURES = {"energy": "Energia", "entropy": "Entropia"}
DECISIONS = {
...
st.set_page_config(page_title="Validação wavelet pulmonar", layout="wide")
st.title("Validação de textura pulmonar")
st.caption("Wavelet · Energia e entropia das sub-bandas de detalhe · O painel só interpreta resultados já calculados.")
required = ["case_validation.csv", "features_wavelet.csv", "reference_originals.csv", "comparison_results.csv"]
...
with st.expander("O que a energia e a entropia wavelet medem?", expanded=False):
    st.markdown(
        "- **Energia:** média dos coeficientes de detalhe ao quadrado. Cresce quando a ROI tem "
        "mais estrutura fina — bordas, reticulação, ruído de alta frequência.\n"
        "- **Entropia:** dispersão dos coeficientes de detalhe. Cresce quando a energia se espalha "
        "por muitos coeficientes, ou seja, textura mais irregular e menos concentrada em poucas bordas.\n"
        "- Ambas resumem as sub-bandas LH, HL e HH de todos os níveis; o CSV de features guarda "
        "também o valor por sub-banda e por nível."
    )
...
for row in comparisons:
    ...
    summary = {"Caso": row["case_id"], "Decisão": title}
    for feature, label in FEATURES.items():
        summary[f"{label}: distância do par"] = percentage(as_float(row[f"relative_delta_{feature}"]))
        summary[f"{label}: referência"] = "Fora" if row[f"synthetic_{feature}_outside_reference"] == "True" else "Dentro"
    summary["Explicação"] = row["decision_reason"]
...
with st.expander("Dados técnicos e validação das imagens", expanded=False):
    ...
    st.markdown("**Todas as features wavelet calculadas**")
