"""Executa o MVP wavelet: seleção, referência, comparação e figuras."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--patch-size", type=int, default=16, help="Lado da janela deslizante, em pixels.")
    parser.add_argument("--patch-stride", type=int, default=4, help="Passo entre janelas, em pixels.")
    parser.add_argument("--min-tissue", type=float, default=0.9, help="Fração mínima de pulmão exigida para aceitar uma janela.")
...
def main() -> None:
    ...
    for case in valid:
        original_metrics = wavelet_energy_entropy(case.original, case.lung_mask, clip_low, clip_high, args.wavelet, args.level, args.patch_size, args.patch_stride, args.min_tissue)
        synthetic_metrics = wavelet_energy_entropy(case.synthetic, case.lung_mask, clip_low, clip_high, args.wavelet, args.level, args.patch_size, args.patch_stride, args.min_tissue)
    ...
    with (output / "reference_originals.json").open("w", encoding="utf-8") as file:
        json.dump({"n_valid_originals": len(originals), "clip_low": clip_low, "clip_high": clip_high, "wavelet": args.wavelet, "level": args.level, "patch_size": args.patch_size, "patch_stride": args.patch_stride, "min_tissue": args.min_tissue, "erosion_radius": args.erosion_radius, "reference": reference}, file, indent=2)
